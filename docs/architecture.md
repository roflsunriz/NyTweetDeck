...
12. 主要10言語と日本語、RTL、文字サイズ、色、密度、動き、メディア表示、動画ループ・音量、自動翻訳ON/OFF、トレンド検索語句と履歴を変更し、レイアウトv7として自動保存・復元する。v1〜v6は既存のカラムと設定を保ったまま強制移行する。
13. 表示中ポストの数字とDMをWeb認証済みLive Pipelineで購読し、切断時だけ指数バックオフで再接続してローカルSSEへ転送する。
...
16. 投稿形式フィルターは取得済みの正規化投稿に対してクライアント側で適用し、切替による追加API消費を発生させない。
...
19. リポスト応答は外側作者を`repostedBy`へ分離し、元ポストをカード本体として正規化する。引用元は作者・本文・言語・日時・メディアを持つ埋め込みポストへ正規化し、Web準拠カードからアプリ内詳細へ接続する。
