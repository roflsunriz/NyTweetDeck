...
12. 主要10言語と日本語、RTL、文字サイズ、色、密度、動き、メディア表示、動画ループ・音量・画質、自動翻訳ON/OFF、投稿の翻訳先言語、タイムライン自動更新、トレンド検索語句と履歴を変更し、サーバー側の単一レイアウトとして自動保存・復元する。全loopbackアドレスは同じリビジョンを読み書きし、別画面の更新はSSE、フォーカス、可視化復帰時に追従する。競合時は古い画面の上書きを拒否して最新設定へ揃える。
13. 表示中ポストの数字とDMをWeb認証済みLive Pipelineで購読する。現行イベントURL、接続時の20 topic、`/system/config`で得るセッション、残りtopicの追加購読、TTL前の更新へ追従し、切断時だけ指数バックオフで再接続してローカルSSEへ転送する。Live Pipelineが通知しない通常タイムラインの新規投稿は、設定で停止・再開でき、オンの場合は可視タブ限定、全カラム15秒以上の共有間隔、変化なしで60秒から最大5分へ延びる適応的な確認で補完する。
...
18. 投稿の`lang`と選択した翻訳先言語が異なる場合は、X公式Webと同じ認証付きStrato翻訳RESTを`translationSource=X`で遅延取得する。表示言語とは独立した翻訳先を設定でき、Google等の外部翻訳へフォールバックせず、結果は上限付きで再利用し、投稿単位の原文表示と歯車による全カラム一括ON/OFFをReact Context経由で同期する。事前翻訳がない新着はIntersection Observerで表示付近に入ってから最大2並列で取得し、429・408・425・5xx・タイムアウトは初回を含め最大4回試行する。サーバーは同一要求を結合し、Xのrate-limit残量・reset時刻・成功率を追跡して残り5%を予備枠として保護する。
