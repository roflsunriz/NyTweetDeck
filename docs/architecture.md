...
13. 主要10言語と日本語、RTL、文字サイズ、色、密度、動き、メディア表示、自動翻訳ON/OFFを変更し、レイアウトv5として自動保存・復元する。
...
19. 投稿の`lang`とUI言語が異なる場合は、X公式Webと同じ認証付きStrato翻訳RESTを`translationSource=X`で遅延取得する。Google等の外部翻訳へフォールバックせず、結果は上限付きで再利用し、投稿単位の原文表示と歯車による全カラム一括ON/OFFをReact Context経由で同期する。
20. リポスト応答は外側作者を`repostedBy`へ分離し、元ポストをカード本体として正規化する。引用元は作者・本文・言語・日時・メディアを持つ埋め込みポストへ正規化し、Web準拠カードからアプリ内詳細へ接続する。
