...
設定画面から、表示言語とは独立した投稿の翻訳先言語、タイムラインの自動更新、動画の既定画質（自動／低／中／高）を選べます。複数画質の動画は再生中の画質メニューでも切り替えられます。各タイムラインカラムの上部では、Xの検索ランキングを使う「トップ」と「最新」を選べます。

「メインナビゲーションを常時表示」をオフにすると左メニューを自動収納できます。収納中は左端から右へスワイプするか左上のメニューボタンで一時表示でき、3秒後に再び収納します。オンへ戻すと常時表示になります。
