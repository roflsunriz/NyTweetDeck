...
設定画面から、表示言語とは独立した投稿の翻訳先言語、タイムラインの自動更新、動画の既定画質（自動／低／中／高）、メインナビゲーションのサイド／下部配置を選べます。複数画質の動画は再生中の画質メニューでも切り替えられます。各タイムラインカラムの上部では、Xの検索ランキングを使う「トップ」と「最新」を選べます。
