...
「もっと見つける」「Xから」のおすすめ投稿は、返信一覧の後に独立した枠で表示します。返信の線やインデントではつながず、選ぶとその投稿の詳細へ進みます。

コミュニティノートも自動翻訳の対象です。同梱訳がない場合はXのライブ翻訳を試し、原文と出典を維持したまま切り替えられます。ノートとポストの成功訳は共通のメモリ管理で再利用し、同じ翻訳の重複要求をまとめます。失敗時は原文を残し、再試行できます。アプリを終了するとキャッシュは消えます。
