...
更新は設定画面の「GitHubから最新版APKをダウンロード」から、GitHub Releases APIで検出した最新の安定版`android-v*`の署名済みAPKをダウンロードフォルダーへ直接保存できる。完了通知からAPKを開き、同じ署名の新しいAPKを上書きする。ログイン状態と設定を維持するため、署名不一致時にアンインストールして続行しない。
