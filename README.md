...
更新時はAndroid版の設定画面で「GitHubから最新版APKをダウンロード」を押すと、GitHub Releases APIから最新の安定版`android-v*`に添付された署名済みAPKを選び、端末のダウンロードフォルダーへ直接保存します。完了通知からAPKを開いて上書きしてください。同じ署名の公式APKであれば、保存済みログイン状態と設定を維持します。署名不一致時はデータを失う自動アンインストールを行わず、インストールを中止してください。
