...
現在はAndroid API接続段階です。カラム型UI、追加・削除・対象設定、アカウント切替、表示設定、暗号化Vault、OAuth付きGraphQL、ホーム・ユーザー・リスト・履歴・検索タイムライン、ページネーション、いいね・リポスト・履歴保存まで製品経路へ接続しています。実アカウントログインはAndroidクライアント資格情報の明示承認待ちです。
