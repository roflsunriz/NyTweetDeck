...
設定を開くと更新を確認し、インストール済みの版が最新なら更新ボタンは無効になります。新版のダウンロード開始後も重複操作を防ぎ、通信に失敗した場合は再試行できます。

更新時はAndroid版の設定画面で「最新版をダウンロード」を押すと、GitHub Releases APIから最新の安定版`android-v*`に添付された署名済みAPKを選び、端末のダウンロードフォルダーへ直接保存します。完了通知からAPKを開いて上書きしてください。同じ署名の公式APKであれば、保存済みログイン状態と設定を維持します。署名不一致時はデータを失う自動アンインストールを行わず、インストールを中止してください。
...
カラムは必要な数だけ追加できます。カラム上部のドラッグハンドルを動かすと並べ替えられ、「-」を押すと削除できます。配置は自動保存され、次回も同じ並びで開きます。
...
| ポストの詳細や返信を見る | ポスト本文またはカードを押す。取得済みの対象ポストは即時表示し、返信と会話コンテキストを背後で読み込む |
| 返信の過去文脈を見る | 返信ポストを開くと、取得できた親返信を先に表示する。下まで進むと追加の返信を読み込む |
| 返信の読み込みが終わったとき・失敗したとき | 最後まで取得すると読み込み表示が消える。取得失敗時は再試行ボタンで読み直せる |
| ポスト詳細の画像を拡大する | 画像を押す。押している間だけドラッグ移動し、スクロールで拡大・縮小。左境界へ移動すると次、右境界へ移動すると前の画像へ切り替わる。Escでポスト詳細へ戻る |
| 複数画像を順に見る | 投稿に含まれる画像の範囲だけを移動する。先頭より前、末尾より後には進まず、画像はループしない |
...
| 返信をさらにたどる | 詳細内の返信本文またはカードを押すと、その返信の詳細と返信一覧を開く。同じ操作でさらに先へ進める。戻る操作で一段ずつ前の会話へ戻る |
| Xのおすすめ投稿を見る | 詳細下部の「もっと見つける」は返信とは別枠。投稿を押すとその投稿の詳細を開く |
...
| ユーザーをミュート／ブロックする | ポストメニューから操作する。成功すると、そのユーザーのポストとリポストを表示中のタイムラインから除外する |
...
| コミュニティノートを翻訳する | 自動翻訳が有効ならXの同梱訳を表示し、なければライブ翻訳を試す。「原文を表示」で切り替えられ、失敗時は原文を維持する |
...
| メニューを自動収納する | 設定の「メニューを常に表示」をオフにする。デスクトップは画面端へのマウスホバー、Androidはサイドから内側または下端から上へのスワイプで一時表示し、3秒後に自動収納する |
| カラムを並べ替える | カラム上部を別のカラム位置へドラッグする |
| カラムの投稿順を変える | カラム上部の「トップ」または「最新」を選ぶ。検索カラムのトップ順はXの検索ランキングを使用する |
| タイムラインの先頭へ戻る | カラム上部の並び順の隣にある上向き矢印を押す。そのカラムだけ先頭へ戻る |
...
自動翻訳は初期状態で有効です。設定の「翻訳先の言語」で表示言語とは別の翻訳先を選べます。選択した言語と異なるポストだけをXが提供する翻訳で表示し、Googleなどの外部翻訳サービスへは送信しません。X側でまだ翻訳されていない新着ポストは、現在の表示範囲に近いものから最大2件ずつ処理します。レート制限や一時的な通信失敗では解除時刻または段階的な待機後に、初回を含め最大4回試行し、最終失敗時だけ手動の「再試行」を表示します。

## 表示と設定を変更する

ノートとポストの翻訳結果はメモリに保持し、同じアカウント・翻訳先で再表示すると再利用します。同時に同じ翻訳を求めた場合も要求をまとめます。失敗した結果は保存せず、アプリの終了やキャッシュ上限を超えた場合は再取得します。

メンション・URLを除くと文字が残らない投稿は自動翻訳を行いません。画像のみ、宛先のユーザーIDと画像だけ、絵文字・数字・記号だけの投稿が対象です。メンションや画像があっても本文に文章があれば翻訳します。引用ポストにも同じ判定を適用します。

メインメニューの設定アイコンから、次を変更できます。

項目は表示・翻訳・メディア・ナビゲーションなどの目的別にまとまっています。詳しい説明や診断情報は、必要なときに開いて確認できます。
...
- X自動翻訳の通信成功率と残り利用枠
- 設定JSONのインポート・エクスポート
- 新版がある場合だけ更新ボタンを有効にし、GitHub Releasesから安定版デスクトップZIPを直接ダウンロード
...
動画は初期ミュートで表示します。シークまたは音量を直接操作するとミュートを自動解除します。Xが返した複数のMP4画質を保持し、既定画質を設定から選べます。動画が再生範囲に入った後はプレイヤー内の画質メニューから一時的に切り替えられます。デスクトップ版は再生範囲の前後1200pxで動画を先読みし、ビューポート上端から中央までの再生範囲だけ自動再生します。自動画質で読み込みが停止した場合は再生位置を保って低いビットレートへ切り替え、先読み範囲を離れた時点で停止して接続を解除します。Android版は同じ可視範囲だけ再生し、範囲外では一時停止して共有ディスクキャッシュと準備済みバッファを再利用します。動画詳細は自動再生OFFでも手動再生でき、ループ設定とミュート解除時の保存済み音量を適用します。
