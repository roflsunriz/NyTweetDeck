...
- Fullscreen APIでプレイヤーがトップレイヤーへ移ると、カラムをrootとしたIntersectionObserverが非交差を返す。全画面の要求中・表示中は動画を解放したり自動再生判定で停止したりしない。終了時は全画面中の古い非交差値を使わず、監視を再設定してカラム内の状態を測り直す。`configured-video-fullscreen.test.tsx`参照。
