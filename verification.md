...
- 「メインナビゲーションを常時表示」をオフにし、デスクトップでは画面端へのホバー、Androidでは左端から右へのスワイプで一時表示されることを確認します。メニュー内の操作中は表示を維持し、ポインター離脱またはAndroidでの表示から3秒後に収納され、設定をオンへ戻すと時間経過後も常設されることを確認します。
