...
設定永続化を変更した場合は、共有ストアが空のときだけ旧`localStorage`を移行すること、既存共有設定をアドレス別の旧値で上書きしないこと、`127.0.0.1`と`localhost`から同じリビジョンを取得できること、同時更新の競合時に古い設定を拒否すること、破損時に`.bak`から復旧することを確認します。
...
統合インストーラーはJARとランチャーを置換前に`.bak`へ退避し、更新後のHTTP/HTTPS確認に失敗した場合は旧ファイルと旧自動起動タスクを復旧して再起動します。更新がローカル検証またはCIで失敗した場合は、失敗ログを保存したうえで更新コミットを戻します。永続データのスキーマを変更した場合は、更新前に作成したバックアップから復旧し、旧バージョンで読み込めることを確認します。共有設定は`settings.json.bak`から自動復旧し、初回移行が失敗した場合は旧`localStorage`を削除せず再試行可能な状態を維持します。
